# -*- coding: utf-8 -*-
"""Quick script to fix Bước 1 in Build_APK_Colab.ipynb"""
...
# Tìm cell code đầu tiên (Bước 1) - source có chứa "ensurepip"
for cell in nb['cells']:
    if cell.get('cell_type') == 'code':
        source = cell.get('source', [])
        if any('ensurepip' in line for line in source):
            print(f"Found Bước 1 cell, replacing source...")
            # Source mới cho Bước 1
            new_source = [
                ...
                "# Bước 1b: Cài Java 17 + lib build TRƯỚC\n",
                ...
                "r = subprocess.run(['java', '-version'], text=True, stderr=subprocess.STDOUT)\n",
                "print('Java:', (r.stdout.splitlines()[0] if r.stdout else 'unknown'))\n",
                "# Bước 1d: Tạo venv với Python 3.11 (uv-installed Python không có ensurepip)\n",
                ...
                "subprocess.run(['/content/venv/bin/pip', 'install', '-q', '--upgrade', 'pip'], check=True)\n",
                "subprocess.run(['/content/venv/bin/pip', 'install', '-q', 'buildozer==1.5.0', 'cython>=3.0'], check=True)\n",
